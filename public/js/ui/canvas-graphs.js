...
class Graph{
    ...
    constructor(
        canvas, abortController, width, height, tabs, bHideEmptyTabs, data, 
        ...
        this.bSkipForcedWholeNums = bSkipForcedWholeNums ?? false;
        ...
        this.titleStartY = (this.tabs.length <= 1) ? 5 : this.tabHeight + 2;

        this.advancedMenu = {
            "startX": 82,
            "startY": (this.tabs.length === 1) ? 0 :this.tabHeight,
            "width": 18,
            "titleFontSize": 3,
            "titleOffsetY": 1,
            "fontSize": 2.7,
    ...
    bTabHaveData(index){
        ...
        for(let i = 0; i < this.data[index].length; i++){
            ...
            for(let x = 0; x < d.length; x++){
                ...
                if(this.minimumDataPoints !== 0){

                    if(d[x].values.length >= this.minimumDataPoints){
                        return true;
                    }

                }else{
                    if(d[x].values.length > 0) return true;
                }
            }
        }
    ...
    removeEmptyTabs(){
        ...
        for(let i = 0; i < this.tabs.length; i++){
            ...
            if(this.bTabHaveData(i)){
                ...
                validLabelPrefixes.push(this.labelsPrefix[i]);
                validLabels.push(this.labels[i]);
                //auto select
                if(this.selectedTab === -1){
                    this.selectedTab = i;
                }
        ...
        this.tabs = validTabs;
        this.data = validData;
        this.labels = validLabels;
        this.labelsPrefix = validLabelPrefixes;
    }
    ...
    setMinMax(){
       ...
       //return;
        ...
        if(!this.bSkipForcedWholeNums && rem !== 0){
    ...
    renderHoverBox(){
        ...
        const titleFontSize = 3.3;
        const titleFontColor = "white";
        const titleBottomPadding = 2.4;
        const fontSize = 2.5;
        const maxNameWidth = 15;
        const fontPadding = 0.7;
        const infoFontSize = 2.8;
        const infoBottomPadding = 0.7;
        const infoFontColor = "rgb(222,222,222)";
        ...
        if(this.info !== null && this.info[this.selectedTab] !== undefined && this.info[this.selectedTab][this.hoverDataIndex] !== undefined){
            infoHeight = fontPadding + infoFontSize + infoBottomPadding;
            info = this.info[this.selectedTab][this.hoverDataIndex];
        }

        const height = titleFontSize + titleBottomPadding + infoHeight + fontPadding + ((fontSize + fontPadding) * enabledKeys);
        ...
        if(info !== null){
            ...
            textY += infoFontSize;
            this.fillText(x + width * 0.5, textY + 2, info, titleFontSize, infoFontColor, "center", "Arial", width);
        }
        ...
        //this.fillText(x, textY - 5, `${this.hoverDataIndex}/${this.currentData[0].values.length}`, fontSize, "white", "left", "Arial", maxNameWidth);
    ...
    renderNoData(){
        ...
        this.fillRect(0,0,100,100, this.style.backgroundColor, false);
        this.fillText(50,30,"No Data", 25, this.style.titleColor, "center", "Arial", 100);
    }
    ...
    render(){
        ...
        this.fillText(50, this.titleStartY, this.tabs[this.selectedTab].title, 5, this.style.titleColor, "center", "Arial", 62);
